...
example {n : ℕ} (i : Fin n) :
    ∃ c : Circuit signature n 1, c.Computes interpretation (fun x => x i) ∧ c.size ≤ 0 := by
...
example : ∃ c : Circuit signature 2 2,
  ...
  obtain ⟨c, hc, hg⟩ := hout.exists_circuit_family
  exact ⟨c, fun x => ⟨by simpa using hc x 0, by simpa using hc x 1⟩, hg⟩
...
example : (fun x : Fin 1 → Bool => x 0) ∈ computableFunctions 1 0 :=
  mem_computableFunctions.mpr ⟨Circuit.id signature 1, fun _ => rfl, le_rfl⟩
