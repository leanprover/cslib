...
This plan restructures the cslib task graph based on both the initial research synthesis and the team research findings. The core goal is unchanged from the prior plan: move ~5,000 lines of reusable BimodalLogic content from bimodal-only tasks (2-11) into standalone Propositional, Modal, and Temporal modules via new tasks (20-23). The team research introduced important refinements: DeductionTheorem should stay per-logic (not in Task 20), Task 22 scope must include ~20 HasAxiom* typeclasses and BimodalTMHilbert compatibility, and a new Task 23 for standalone temporal semantics is needed. All changes are edits to specs/state.json and specs/TODO.md -- no Lean code is written.

Definition of done: state.json and TODO.md reflect the modular factoring with tasks 20-23 created, tasks 2-7 and 12 revised, and the Task Order section updated with the new dependency graph. Seed research reports exist for new tasks.
...
The prior plan (01_modular-factoring.md) had 4 phases: design task graph, create tasks 20-22, revise tasks 2-7/12, verify consistency. The structure was sound but predated the team research. Key lessons:
- **Effort calibration**: 4 hours was reasonable; this revision is slightly smaller at 3 hours since the design phase is absorbed into the plan itself.
- **Validated approaches**: Phase structure (create new tasks -> revise existing -> verify) is correct. The dependency graph design (Waves 1-5) was directionally right but needs updates for Task 23 and DeductionTheorem changes.
- **Risks encountered**: Task 22 scope underestimation was confirmed by team research. BimodalTMHilbert compatibility was not in prior plan.

### Roadmap Alignment

ROADMAP.md exists but contains only the default template with no items. No roadmap items to align against.
...
- Update the Task Order section in TODO.md with the revised dependency graph
...
- Restructuring tasks 15-18 (independent infrastructure/meta tasks)
- Modifying ROADMAP.md (it has no items to update)
...
**Goal**: Update task descriptions and dependencies for existing tasks to reflect the modular factoring, incorporating team research refinements.
...
**Goal**: Regenerate the Task Order section of TODO.md and perform final consistency checks across state.json and TODO.md.
...
- [ ] TODO.md Task Order section reflects the correct 6-wave dependency graph
- [ ] No circular dependencies in the task graph
...
- `specs/TODO.md` (updated with 4 new task entries + revised descriptions + new Task Order)
- `specs/020_propositional_hilbert_theorems/reports/01_seed-research.md` (new)
- `specs/021_modal_proof_system_theorems/reports/01_seed-research.md` (new)
- `specs/022_temporal_infrastructure_theorems/reports/01_seed-research.md` (new)
- `specs/023_temporal_semantics_linear_orders/reports/01_seed-research.md` (new)
...
All changes target specs/ files only (state.json, TODO.md, seed research reports). No Lean source files are modified. If the restructuring proves problematic:
1. Revert to the pre-implementation commit: `git checkout HEAD -- specs/state.json specs/TODO.md`
