...
This module defines the Context type used to represent assumptions
in derivations for bimodal logic.
...
- Context is simply `List (Formula Atom)`, leveraging Lean's
  built-in list operations
- Parameterized over a generic `Atom` type for composability
- The `map` operation is essential for modal K and temporal K
  inference rules
-/
...
Used in the derivability relation `Γ ⊢ φ` where `Γ` is a context
of assumptions.
-/
...
-/
def map (f : Formula Atom → Formula Atom) :
    Context Atom → Context Atom := List.map f
...
theorem map_length (f : Formula Atom → Formula Atom)
    (Γ : Context Atom) :
    (map f Γ).length = Γ.length := by
  simp [map]

/-- Mapping functions compose. -/
theorem map_comp (f g : Formula Atom → Formula Atom)
    (Γ : Context Atom) :
...
theorem map_nil (f : Formula Atom → Formula Atom) :
    map f [] = [] := by
  rfl

/-- Mapping distributes over cons. -/
theorem map_cons (f : Formula Atom → Formula Atom)
    (φ : Formula Atom) (Γ : Context Atom) :
    map f (φ :: Γ) = f φ :: map f Γ := by
  rfl

/-- Mapping distributes over append. -/
theorem map_append (f : Formula Atom → Formula Atom)
    (Γ Δ : Context Atom) :
    map f (Γ ++ Δ) = map f Γ ++ map f Δ := by
  simp [map]

/-- Membership in mapped context comes from mapping a member. -/
theorem mem_map_iff {f : Formula Atom → Formula Atom}
    {Γ : Context Atom} {φ : Formula Atom} :
    φ ∈ map f Γ ↔ ∃ ψ ∈ Γ, f ψ = φ := by
  simp [map]

/-- If `ψ ∈ Γ`, then `f ψ ∈ map f Γ`. -/
theorem mem_map_of_mem {f : Formula Atom → Formula Atom}
    {Γ : Context Atom} {ψ : Formula Atom}
    (h : ψ ∈ Γ) : f ψ ∈ map f Γ := by
  rw [mem_map_iff]
  exact ⟨ψ, h, rfl⟩

/-- Empty context has no members. -/
theorem not_mem_nil (φ : Formula Atom) :
    φ ∉ ([] : Context Atom) := by
  simp
...
theorem isEmpty_iff_eq_nil (Γ : Context Atom) :
    isEmpty Γ = true ↔ Γ = [] := by
  cases Γ with
  | nil => simp [isEmpty]
...
theorem exists_mem_of_ne_nil {Γ : Context Atom}
    (h : Γ ≠ []) :
    ∃ φ, φ ∈ Γ := by
  cases Γ with
  | nil => contradiction
