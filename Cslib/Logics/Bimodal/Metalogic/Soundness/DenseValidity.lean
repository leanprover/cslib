...
theorem swap_axiom_mt_valid (φ : Formula Atom) :
    is_valid D ((Formula.box φ).imp φ).swapTemporal := by
...
theorem swap_axiom_m4_valid (φ : Formula Atom) :
    is_valid D ((Formula.box φ).imp (Formula.box (Formula.box φ))).swapTemporal := by
...
theorem swap_axiom_mb_valid (φ : Formula Atom) :
    is_valid D (φ.imp (Formula.box φ.diamond)).swapTemporal := by
  intro ℱ M Omega _h_sc τ h_mem t
...
theorem swap_axiom_ta_valid (φ : Formula Atom) :
    is_valid D (φ.imp (Formula.allFuture φ.somePast)).swapTemporal := by
  intro ℱ M Omega _h_sc τ _h_mem t
  unfold Formula.swapTemporal truth_at
...
theorem swap_axiom_tl_valid (φ : Formula Atom) :
    is_valid D (φ.always.imp (Formula.allFuture (Formula.allPast φ))).swapTemporal := by
  intro ℱ M Omega _h_sc τ _h_mem t
  unfold Formula.swapTemporal truth_at
  simp only [Formula.always, Formula.and, Formula.swapTemporal, Formula.neg, truth_at]
  ...
  have h_future : ∀ r, t < r → truth_at M Omega τ r φ.swapTemporal := by
    by_contra h_not; push_neg at h_not
    obtain ⟨r, htr, h_neg⟩ := h_not
    exact h_always fun h_G _ => h_G ⟨r, htr, h_neg, fun _ _ _ hf => absurd hf not_false⟩
  have h_present : truth_at M Omega τ t φ.swapTemporal := by
    by_contra h_not
    exact h_always fun _ h_inner => h_inner (fun h_pres _ => h_not h_pres)
  have h_past : ∀ r, r < t → truth_at M Omega τ r φ.swapTemporal := by
    by_contra h_not; push_neg at h_not
...
theorem swap_axiom_F_until_equiv_valid (φ : Formula Atom) :
    is_valid D ((Formula.someFuture φ).imp
      (Formula.untl φ (Formula.bot.imp Formula.bot))).swapTemporal := by
  intro ℱ M Omega _h_sc τ _h_mem t
  simp only [Formula.swapTemporal, truth_at, Formula.somePast, Formula.someFuture,
...
theorem swap_axiom_P_since_equiv_valid (φ : Formula Atom) :
    is_valid D ((Formula.somePast φ).imp
...
theorem swap_axiom_mf_valid (φ : Formula Atom) :
    is_valid D ((Formula.box φ).imp (Formula.box (Formula.allFuture φ))).swapTemporal := by
...
theorem mp_preserves_swap_valid (φ ψ : Formula Atom)
    (h_imp : is_valid D (φ.imp ψ).swapTemporal)
    (h_phi : is_valid D φ.swapTemporal) :
    is_valid D ψ.swapTemporal := by
...
theorem modal_k_preserves_swap_valid (φ : Formula Atom)
    (h : is_valid D φ.swapTemporal) :
    is_valid D (Formula.box φ).swapTemporal := by
  intro ℱ M Omega h_sc τ _h_mem t
  simp only [Formula.swapTemporal, truth_at]
  intro σ h_σ_mem
...
theorem temporal_k_preserves_swap_valid (φ : Formula Atom)
    (h : is_valid D φ.swapTemporal) :
    is_valid D (Formula.allFuture φ).swapTemporal := by
  intro ℱ M Omega h_sc τ h_mem t
  unfold Formula.swapTemporal truth_at
  intro ⟨s, hst, h_neg, _⟩
...
theorem axiom_swap_valid (φ : Formula Atom) (h : Axiom φ) [DenselyOrdered D] [Nontrivial D]
    (h_fc : h.minFrameClass ≤ FrameClass.Dense) : is_valid D φ.swapTemporal := by
  cases h with
  | imp_k ψ χ ρ =>
    intro ℱ M Omega _h_sc τ _h_mem t
    simp only [Formula.swapTemporal, truth_at]
  ...
  | peirce ψ χ =>
    ...
    · exact h
    · have h_imp : truth_at M Omega τ t (ψ.swapTemporal.imp χ.swapTemporal) := by
        unfold truth_at
        intro h_psi
        exfalso
        exact h h_psi
  ...
  | left_mono_until_G φ χ ψ =>
    ...
    have h_imp_r : truth_at M Omega τ r φ.swapTemporal → truth_at M Omega τ r χ.swapTemporal := by
      intro h_φr
      by_contra h_neg
      apply h_H
  ...
  | absorb_until φ ψ =>
    ...
    have h_φs₁_and_since : truth_at M Omega τ s₁ φ.swapTemporal ∧
  ...
  | absorb_since φ ψ =>
    ...
    have h_φs₁_and_until : truth_at M Omega τ s₁ φ.swapTemporal ∧
        (∃ s₂, s₁ < s₂ ∧ truth_at M Omega τ s₂ ψ.swapTemporal ∧
          ∀ q, s₁ < q → q < s₂ → truth_at M Omega τ q φ.swapTemporal) := by
      constructor
      · by_contra h_neg; exact h_conj (fun h_φ _ => h_neg h_φ)
  ...
  | temp_linearity φ ψ =>
    ...
    simp only [Formula.swapTemporal, Formula.and, Formula.or, Formula.neg, truth_at]
    intro h_conj
    have ⟨s1, hs1t, h_φs1⟩ : ∃ s, s < t ∧ truth_at M Omega τ s φ.swapTemporal := by
      by_contra h_no; push_neg at h_no
      exact h_conj (fun ⟨s, hst, h_phi, _⟩ _ => absurd h_phi (h_no s hst))
    have ⟨s2, hs2t, h_ψs2⟩ : ∃ s, s < t ∧ truth_at M Omega τ s ψ.swapTemporal := by
      by_contra h_no; push_neg at h_no
  ...
  | temp_linearity_past φ ψ =>
    ...
    simp only [Formula.swapTemporal, Formula.and, Formula.or, Formula.neg, truth_at]
    intro h_conj
    have ⟨s1, hts1, h_φs1⟩ : ∃ s, t < s ∧ truth_at M Omega τ s φ.swapTemporal := by
      by_contra h_no; push_neg at h_no
      exact h_conj (fun ⟨s, hts, h_phi, _⟩ _ => absurd h_phi (h_no s hts))
    have ⟨s2, hts2, h_ψs2⟩ : ∃ s, t < s ∧ truth_at M Omega τ s ψ.swapTemporal := by
      by_contra h_no; push_neg at h_no
  ...
  | dense_indicator =>
    intro ℱ M Omega _h_sc τ _h_mem t
    simp only [Formula.swapTemporal, Formula.neg, truth_at]
...
theorem axiom_temp_linearity_valid (φ ψ : Formula Atom) :
    is_valid D (Formula.and (Formula.someFuture φ) (Formula.someFuture ψ) |>.imp
...
theorem axiom_temp_linearity_past_valid (φ ψ : Formula Atom) :
    is_valid D (Formula.and (Formula.somePast φ) (Formula.somePast ψ) |>.imp
...
theorem axiom_density_valid [DenselyOrdered D] (φ : Formula Atom) :
    is_valid D (φ.allFuture.allFuture.imp φ.allFuture) := by
  intro ℱ M Omega _h_sc τ _h_mem t
  unfold truth_at
...
theorem axiom_modal_future_valid (φ : Formula Atom) :
    is_valid D ((φ.box).imp ((φ.allFuture).box)) := by
  intro ℱ M Omega h_sc τ _h_mem t
  unfold truth_at
...
theorem temporal_necessitation_preserves_local_valid {φ : Formula Atom}
    (h : is_valid D φ) :
    is_valid D (Formula.allFuture φ) := by
  intro ℱ M Omega h_sc τ h_mem t
  unfold truth_at
  intro ⟨s, hts, h_neg, _⟩
  exact h_neg (h ℱ M Omega h_sc τ h_mem s)
