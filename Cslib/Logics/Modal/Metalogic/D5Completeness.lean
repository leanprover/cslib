...
theorem d5_completeness (φ : Proposition Atom)
  ...
  have h_cons : Modal.SetConsistent (@D5Axiom Atom)
      ({Proposition.neg φ} : Set (Proposition Atom)) := by
    intro L hL
    unfold Metalogic.Consistent
    intro ⟨d⟩
    have d_weak : DerivationTree (@D5Axiom Atom) [Proposition.neg φ]
        Proposition.bot :=
    ...
    have efq_ax : DerivationTree (@D5Axiom Atom) (Atom := Atom) []
        (Proposition.bot.imp φ) :=
      .ax [] _ (.efq φ)
    have ik : DerivationTree (@D5Axiom Atom) (Atom := Atom) []
    ...
    have is_ax : DerivationTree (@D5Axiom Atom) (Atom := Atom) []
    ...
    have peirce_ax : DerivationTree (@D5Axiom Atom) (Atom := Atom) []
