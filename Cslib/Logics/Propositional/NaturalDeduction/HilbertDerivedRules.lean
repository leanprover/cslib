...
noncomputable def hilbertNegI {Γ : List (PL.Proposition Atom)}
    ...
    (d : DerivationTree PropositionalAxiom (A :: Γ) Proposition.bot) :
    DerivationTree PropositionalAxiom Γ (Proposition.neg A) :=
  impI d
...
def hilbertNegE {Γ : List (PL.Proposition Atom)}
    {A : PL.Proposition Atom}
    (d₁ : DerivationTree PropositionalAxiom Γ (Proposition.neg A))
    (d₂ : DerivationTree PropositionalAxiom Γ A) :
    DerivationTree PropositionalAxiom Γ Proposition.bot :=
  impE d₁ d₂
...
def hilbertTopI {Γ : List (PL.Proposition Atom)} :
    DerivationTree PropositionalAxiom Γ (Proposition.top : PL.Proposition Atom) :=
...
def hilbertDne {Γ : List (PL.Proposition Atom)}
    {A : PL.Proposition Atom}
    (d : DerivationTree PropositionalAxiom Γ (Proposition.neg (Proposition.neg A))) :
    DerivationTree PropositionalAxiom Γ A := by
  -- d : Gamma |- (A -> bot) -> bot
...
noncomputable def hilbertAndI {Γ : List (PL.Proposition Atom)}
    ...
    (d₁ : DerivationTree PropositionalAxiom Γ A)
    (d₂ : DerivationTree PropositionalAxiom Γ B) :
    DerivationTree PropositionalAxiom Γ (A.and B) := by
...
def hilbertAndE1 {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (d : DerivationTree PropositionalAxiom Γ (A.and B)) :
    DerivationTree PropositionalAxiom Γ A := by
...
def hilbertAndE2 {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (d : DerivationTree PropositionalAxiom Γ (A.and B)) :
    DerivationTree PropositionalAxiom Γ B := by
  -- d : Gamma |- (A -> (B -> bot)) -> bot
...
noncomputable def hilbertOrI1 {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (d : DerivationTree PropositionalAxiom Γ A) :
    DerivationTree PropositionalAxiom Γ (A.or B) := by
  -- Goal: Gamma |- (A -> bot) -> B
  apply impI
  -- (A -> bot) :: Gamma |- B
  apply botE
...
def hilbertOrI2 {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (d : DerivationTree PropositionalAxiom Γ B) :
    DerivationTree PropositionalAxiom Γ (A.or B) :=
...
noncomputable def hilbertOrE {Γ : List (PL.Proposition Atom)}
    ...
    (d : DerivationTree PropositionalAxiom Γ (A.or B))
    (dA : DerivationTree PropositionalAxiom (A :: Γ) C)
    (dB : DerivationTree PropositionalAxiom (B :: Γ) C) :
    DerivationTree PropositionalAxiom Γ C := by
  -- d : Gamma |- (A -> bot) -> B
  -- Step 1: Gamma |- A -> C
  have hAC : DerivationTree PropositionalAxiom Γ (A.imp C) := impI dA
  -- Step 2: Gamma |- B -> C
  have hBC : DerivationTree PropositionalAxiom Γ (B.imp C) := impI dB
  -- Step 3: Gamma |- (A -> bot) -> C (compose d with hBC)
  -- Using impI: assume (A -> bot), derive B from d, then C from hBC
  have hNAC : DerivationTree PropositionalAxiom Γ (Proposition.neg A |>.imp C) := by
    apply impI
    -- (A -> bot) :: Gamma |- C
    apply impE (A := B)
  ...
  have hContra : DerivationTree PropositionalAxiom (Proposition.neg C :: Γ) (Proposition.neg A) := by
    apply hilbertNegI
    -- A :: (C -> bot) :: Gamma |- bot
    apply impE (A := C)
  ...
  have hC : DerivationTree PropositionalAxiom (Proposition.neg C :: Γ) C :=
    impE
...
noncomputable def hilbertIffI {Γ : List (PL.Proposition Atom)}
    ...
    (d₂ : DerivationTree PropositionalAxiom Γ (B.imp A)) :
    DerivationTree PropositionalAxiom Γ (A.iff B) :=
  hilbertAndI d₁ d₂
...
def hilbertIffE1 {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (d : DerivationTree PropositionalAxiom Γ (A.iff B)) :
...
def hilbertIffE2 {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (d : DerivationTree PropositionalAxiom Γ (A.iff B)) :
    DerivationTree PropositionalAxiom Γ (B.imp A) :=
  hilbertAndE2 d
...
theorem hilbertNegIDeriv {Γ : List (PL.Proposition Atom)}
    {A : PL.Proposition Atom}
    (h : Deriv PropositionalAxiom (A :: Γ) Proposition.bot) :
    Deriv PropositionalAxiom Γ (Proposition.neg A) := by
  obtain ⟨d⟩ := h; exact ⟨hilbertNegI d⟩

/-- Negation elimination at the `Deriv` level. -/
theorem hilbertNegEDeriv {Γ : List (PL.Proposition Atom)}
    {A : PL.Proposition Atom}
    (h₁ : Deriv PropositionalAxiom Γ (Proposition.neg A))
    (h₂ : Deriv PropositionalAxiom Γ A) :
    Deriv PropositionalAxiom Γ Proposition.bot := by
...
theorem hilbertTopIDeriv {Γ : List (PL.Proposition Atom)} :
    Deriv PropositionalAxiom Γ (Proposition.top : PL.Proposition Atom) :=
  ⟨hilbertTopI⟩
...
theorem hilbertAndIDeriv {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (h₁ : Deriv PropositionalAxiom Γ A)
    (h₂ : Deriv PropositionalAxiom Γ B) :
    Deriv PropositionalAxiom Γ (A.and B) := by
...
theorem hilbertAndE1Deriv {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (h : Deriv PropositionalAxiom Γ (A.and B)) : Deriv PropositionalAxiom Γ A := by
...
theorem hilbertAndE2Deriv {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (h : Deriv PropositionalAxiom Γ (A.and B)) : Deriv PropositionalAxiom Γ B := by
...
theorem hilbertOrI1Deriv {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (h : Deriv PropositionalAxiom Γ A) : Deriv PropositionalAxiom Γ (A.or B) := by
...
theorem hilbertOrI2Deriv {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (h : Deriv PropositionalAxiom Γ B) : Deriv PropositionalAxiom Γ (A.or B) := by
...
theorem hilbertOrEDeriv {Γ : List (PL.Proposition Atom)}
    {A B C : PL.Proposition Atom}
    (h : Deriv PropositionalAxiom Γ (A.or B))
    (hA : Deriv PropositionalAxiom (A :: Γ) C)
    (hB : Deriv PropositionalAxiom (B :: Γ) C) :
    Deriv PropositionalAxiom Γ C := by
...
theorem hilbertDneDeriv {Γ : List (PL.Proposition Atom)}
    {A : PL.Proposition Atom}
    (h : Deriv PropositionalAxiom Γ (Proposition.neg (Proposition.neg A))) :
...
theorem hilbertIffIDeriv {Γ : List (PL.Proposition Atom)}
    ...
    (h₂ : Deriv PropositionalAxiom Γ (B.imp A)) :
    Deriv PropositionalAxiom Γ (A.iff B) := by
...
theorem hilbertIffE1Deriv {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (h : Deriv PropositionalAxiom Γ (A.iff B)) :
...
theorem hilbertIffE2Deriv {Γ : List (PL.Proposition Atom)}
    {A B : PL.Proposition Atom}
    (h : Deriv PropositionalAxiom Γ (A.iff B)) :
    Deriv PropositionalAxiom Γ (B.imp A) := by
