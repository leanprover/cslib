...
- `Derivation.contra` : deriving anything from contradictory hypotheses.
...
def Theory.Derivation.weak {T T' : Theory Atom} {Γ Δ : Ctx Atom} {A : Proposition Atom}
  ...
  | impI D => impI <| D.weak hTheory <| Finset.insert_subset_insert _ hCtx
...
def Theory.Derivation.substAtom {Atom Atom' : Type u} [DecidableEq Atom] [DecidableEq Atom']
  ...
  | andI D₁ D₂ => andI (D₁.substAtom f) (D₂.substAtom f)
  ...
  | impI D => impI <| (Finset.image_insert (· >>= f) _ _) ▸ (D.substAtom f)
...
/-! ### Ex falso and contradiction -/

/-- Ex falso quodlibet as a derived rule. This coincides with the primitive `efq` constructor;
the wrapper is kept, with an explicit conclusion argument, for readability at call sites and for
parity with the pre-primitive-`efq` design where `efqRule` had to thread through an intuitionistic
typeclass. -/
def Theory.Derivation.efqRule {Γ : Ctx Atom} (A : Proposition Atom) (D : T⇓(Γ ⊢ ⊥)) :
    T⇓(Γ ⊢ A) := efq D

/-- Prove any proposition from contradictory hypotheses. -/
def Theory.Derivation.contra {Γ : Ctx Atom} (A B : Proposition Atom) (hΓ : A ∈ Γ)
    (hΓ' : (¬A) ∈ Γ) : T⇓(Γ ⊢ B) :=
  efqRule B <| impE (ass hΓ') (ass hΓ)
