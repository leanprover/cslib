...
4. The truth lemma (`chronicle_truth_lemma`) establishes that satisfaction in
   ...
   function.
5. Applying validity gives `φ ∈ limit_f(0) = M`, contradicting `φ ∉ M`.
...
theorem completeness [Denumerable (Formula Atom)] {φ : Formula Atom}
  ...
  -- The chronicle construction produces limit_dom and limit_f with:
  --   0 ∈ limit_dom, limit_f(0) = M, C0/C4/C5 satisfaction.
  ...
  let model := Metalogic.Chronicle.chronicle_model M hM_mcs
  let t₀ : D := Metalogic.Chronicle.chronicle_zero M hM_mcs
  -- Apply validity: φ is true at t₀ in the chronicle model.
  have h_sat := h_valid D model t₀
  -- By the truth lemma: Satisfies model t₀ φ ↔ φ ∈ limit_f M hM_mcs t₀.val
  have h_mem := (Metalogic.Chronicle.chronicle_truth_lemma M hM_mcs t₀ φ).mp h_sat
  -- t₀.val = 0 and limit_f(0) = M, so φ ∈ M.
