...
* `parEtaC_of_fullEta` : a single η-step gives a count-`1` derivation;
...
theorem ParEtaC.refl {M : Term Var} (h_lc : LC M) : ParEtaC 0 M M := by
  ...
  | LC.abs xs t h => exact .abs xs (fun x hx => ih _ (by rw [size_open_fvar]; grind) (h x hx) rfl)
...
theorem ParEtaC.step_lc_l {n : ℕ} {M N : Term Var} (h : ParEtaC n M N) : LC M := by
  ...
  | app _ _ ihM ihN => exact LC.app ihM ihN
  | abs xs _ ih => exact LC.abs xs _ (fun x hx => (ih x hx))
  | eta hM hMM' ih => exact LC.abs ∅ _ (fun y _ => LC.app (by grind) (by grind))
