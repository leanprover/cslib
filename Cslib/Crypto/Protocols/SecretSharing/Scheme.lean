...
A secret-sharing scheme bundles both the deterministic sharing/reconstruction
interface and the distribution on randomness. Correctness is part of the scheme
interface; privacy is defined separately in
`Cslib.Crypto.Protocols.SecretSharing.Defs`.

## Main definitions

- `Cslib.Crypto.Protocols.SecretSharing.Scheme`:
  a secret-sharing scheme with correctness
...
secret from the shares generated using any randomness seed.
-/
structure Scheme (Secret Randomness Party Share : Type*) [DecidableEq Party] where
