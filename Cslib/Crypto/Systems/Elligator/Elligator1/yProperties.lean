  ...
  Y^2 = X^5 + (r^2 - 2) * X^3 + X := by
    ...
    have h_X_expand_eq_chi_v_mul_v : X^5 + (r^2 - 2) * X^3 + X = χ v * v := by
      ...
      X^5 + (r^2 - 2) * X^3 + X = χ v * (u^5 + (r^2 - 2) * u^3 + u) := by
        ...
        ring_nf
      _ = χ v * v := by rfl
    have h_chi_v_mul_v_isSquare := χ_a_mul_a_IsSquare hv_ne_zero hq_card hq_mod
    have h_chi_v_mul_v_fixed : (χ v * v)^((q + 1) / 2) = χ v * v :=
      a_pow_q_add_one_div_two_eq_a h_chi_v_mul_v_isSquare hq_card hq_mod
    ...
    have h_Y_sq_eq_chi_v_mul_v : Y^2 = χ v * v := by
      calc
        ...
        _ = (χ v * v)^((q + 1) / 2) * 1 := by
          ...
          rw [mul_one]
        _ = χ v * v := by rw [h_chi_v_mul_v_fixed, mul_one]
...
lemma curve_equation
  ...
  x^2 + y^2 = 1 + d * x^2 * y^2 := by
    ...
    have h_Y_sq_mul_one_sub_d_mul_x_sq_eq : Y^2 * (1 - d * x^2) = X * (r * X + (1 + X)^2)^2 := by
      calc
