...
lemma χ_eq_pow (a : F) (hq_card : Fintype.card F = q) (hq_mod : q % 4 = 3) :
    χ a = a ^ ((q - 1) / 2) := by
  have h : (q - 1) / 2 = Fintype.card F / 2 := by
    rw [hq_card]
    omega
  rw [χ, h]
  exact quadraticChar_eq_pow_of_char_ne_two' (ringChar_ne_two hq_card hq_mod) a
...
lemma χ_χ_eq_χ {a : F} (hq_card : Fintype.card F = q) (hq_mod : q % 4 = 3) :
    χ (χ a) = χ a := by
  rcases χ_values (a := a) with h | h | h
  · rw [h, χ_zero]
...
lemma a_pow_q_add_one_div_two_eq_χ_of_a_mul_a {a : F}
    ...
    a ^ ((q + 1) / 2) = (χ a) * a := by
  rw [χ_eq_pow a hq_card hq_mod, ← pow_succ]
  congr 1
  omega
