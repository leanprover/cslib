...
We also provide the `PolyTimeComputable.normalize` operation, which renormalizes a machine's time
bound to the (automatically monotone) evaluation of its bounding polynomial.

The concrete machine constructions witnessing computability of the identity and of composition
(`idComputer`/`PolyTimeComputable.id`, `compComputer`/`PolyTimeComputable.comp`) live in the
`PolyTime/Id` and `PolyTime/Comp` files of this directory.
