...
def _root_.Cslib.Automata.εNA.FinAcc.toNTM (a : εNA.FinAcc State Symbol) : SingleTapeNTM (Option State) Symbol :=
  let a' := a.toSingleAccept
  {
    start := a'.start
    accept := a'.accept
    Tr
      | s, ⟨x, none, Turing.Dir.right⟩, s' => a'.Tr s x s'
      | _, _, _ => False
    accept_halting := sorry
  }

open Acceptor in
theorem pippo {a : εNA.FinAcc State Symbol} : language a.toNTM = language a := by
  sorry
