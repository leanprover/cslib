...
(`MultiTapeTM.spaceUsedByTape`, `MultiTapeTM.spaceUsed`) that are not needed to state the core
definitions in `Deterministic.lean`, but are reusable building blocks for counting and bounding
arguments elsewhere (e.g. `ConfigBound.lean`).

The key geometric facts are `MultiTapeTM.natAbs_le_spaceUsedByTape_of_mem_visited` and
`MultiTapeTM.content_natAbs_le_spaceUsedByTape`: starting from the all-blank tapes with every head
at `0` and moving by at most one cell per step, a computation in which tape `i` has visited at most
`sᵢ` cells keeps that tape's head position and every non-blank cell within `sᵢ` of the origin.
-/
...
lemma visitedByTapeHead_mono (cfg : Cfg k Symbol State input) (i : Fin k)
    {t t' : ℕ} (h : t ≤ t') :
    tm.visitedByTapeHead cfg t i ⊆ tm.visitedByTapeHead cfg t' i := by
  apply Finset.image_subset_image
  grind

/-- Discrete intermediate value: Starting from configuration `cfg`, every position between the
initial head position of tape `i` and the one after `t` steps is part of the "visited set" at
step `t`. -/
lemma uIcc_subset_visitedByTapeHead (cfg : Cfg k Symbol State input) (i : Fin k) (t : ℕ) :
...
head's starting position: by the discrete intermediate value property, the whole interval between
the two positions has been visited, and `spaceUsedByTape` counts the visited positions. -/
lemma natAbs_le_spaceUsedByTape_of_mem_visited
  ...
  have h1 := Finset.card_le_card
    ((tm.uIcc_subset_visitedByTapeHead cfg i t').trans
