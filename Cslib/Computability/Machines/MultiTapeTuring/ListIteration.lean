...
-- TODO continue here. check if this makes sense and then
-- represent combineOr as computes_function

public def computes_function {k : ℕ} (i j : Fin k) (tm : MultiTapeTM k Char)
  (f : Data → Data) : Prop :=
  ∀ views, tm.eval_struct views = some (Function.update views j
      -- TODO do this with if
      match (views i).current with
      | some d => (views j).pushList (f d)
      | none => views j)
