...
/-
# Progress theorem for System F<:

This module proves the progress theorem for System F<:.
Progress states that well-typed expressions in the empty context are either values
...
/-- Progress theorem: well-typed expressions in empty contexts either are values
    or can take a reduction step. -/
theorem progress
  (ht : HasType Γ t T) (hem : Ctx.IsEmpty Γ) :
  (∃ t', Reduce t t') ∨ t.IsVal := by
  induction ht
  case var hb => cases hem; cases hb
  case sub ih => aesop
  ...
  case app ih1 ih2 =>
    ...
    case inl ih1 =>
      have ⟨t1', ih1⟩ := ih1
      left; constructor
      apply Reduce.red_app_fun ih1
    case inr ih1 =>
      rename_i hvt _ _
      have ⟨T0, e0, h⟩ := HasType.value_typing_arrow_inv ih1 hvt
      cases h
      left; constructor
