...
def finLoop_total (h : ∃ s0, s0 ∈ na.start) : na.finLoop.Total where
  next s x := inr (inr ())
  total s x := by
    cases s
    case inl _ => simpa [finLoop, loop, NA.totalize, LTS.totalize]
    case inr _ => grind [finLoop, loop, NA.totalize, LTS.totalize]
...
theorem loop_language_eq [Inhabited Symbol] (h : ¬ language na = 0) :
  ...
  · rintro ⟨s, _, t, h_acc, h_mtr⟩
    ...
    · have h0 : ∃ s0, s0 ∈ na.start := by
        obtain ⟨_, s0, _, _⟩ := nonempty_iff_ne_empty.mpr h
        use s0
      obtain ⟨xs, ss, h_ωtr, rfl, rfl⟩ := LTS.Total.mTr_ωTr (finLoop_total h0) h_mtr
