...
/-- Finset-based version: if the oracles indexed by `S` produce `|S|`-many distinct evaluation
    results, then some oracle in `S` makes at least `⌈log₂ |S|⌉` queries. -/
private theorem exists_mem_queriesOn_ge_clog
    {ι : Type} (t : QueryTree Q Bool α) (S : Finset ι) (hS : S.Nonempty)
    (oracles : ι → (Q → Bool))
    (h_inj : Set.InjOn (fun i => t.eval (oracles i)) ↑S) :
    ∃ i ∈ S, t.queriesOn (oracles i) ≥ Nat.clog 2 S.card := by
  ...
  | query q cont ih =>
    ...
    · push_neg at hle
      -- Find b : Bool such that S.filter (oracles · q = b) has ≥ ⌈|S|/2⌉ elements
      have ⟨b, hb⟩ : ∃ b : Bool,
          (S.card + 1) / 2 ≤ (S.filter (fun i => oracles i q = b)).card := by
        by_contra h; push_neg at h
        have ht := h true; have hf := h false
        have hpart : (S.filter (fun i => oracles i q = true)).card +
            (S.filter (fun i => oracles i q = false)).card = S.card := by
          have := Finset.card_filter_add_card_filter_not (s := S) (fun i => oracles i q = true)
          rwa [show S.filter (fun i => ¬(oracles i q = true)) =
            S.filter (fun i => oracles i q = false) from
            Finset.filter_congr fun i _ => by cases oracles i q <;> simp] at this
        omega
      ...
      calc Nat.clog 2 S.card
          ≤ 1 + Nat.clog 2 S'.card := by
            rw [Nat.clog_of_two_le (by omega) (by omega)]
            have := Nat.clog_mono_right 2 (show (S.card + 2 - 1) / 2 ≤ S'.card by omega)
            omega
