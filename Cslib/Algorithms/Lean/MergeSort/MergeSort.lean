...
/-- Sorts a list using the `mergeSort` algorithm, counting comparisons as time cost.
...
theorem sorted_merge {l1 l2 : List α} (hxs : IsSortedAscending l1) (hys : IsSortedAscending l2) :
    IsSortedAscending ⟪merge l1 l2⟫ := by
  fun_induction merge l1 l2 with
  | case3 =>
    grind [pairwise_cons]
  | _ => simpa

theorem mergeSort_sorted (xs : List α) : IsSortedAscending ⟪mergeSort xs⟫ := by
...
theorem mergeSort_correct (xs : List α) : IsSortedAscending ⟪mergeSort xs⟫ ∧ ⟪mergeSort xs⟫ ~ xs :=
...
/-- Recurrence relation for the time complexity of `mergeSort`.
...
private lemma some_algebra (n : ℕ) :
    ...
    (n + 2) * clog 2 (n + 2) := by
  -- 1. Substitution: Let `N = n + 2` to clean up the expression.
  let N := n + 2
...
/-- Upper bound function for `mergeSort` time complexity: `T(n) = n * ⌈log₂ n⌉`. -/
