...
whose intermediate vertices are introduced by the first `k` phases.  The theorem
...
/--
Restricted paths used by the Floyd-Warshall invariant.

`RestrictedPath k i j` is the family of paths from `i` to `j` generated after
the first `k` phases.  The constructor `through` is the Floyd-Warshall
decomposition through the newly available pivot vertex.
-/
inductive RestrictedPath {n : Nat} : Nat → Fin n → Fin n → Type
  | edge (i j : Fin n) : RestrictedPath 0 i j
  | keep {k : Nat} {i j : Fin n} :
      RestrictedPath k i j → RestrictedPath (k + 1) i j
  | through {k : Nat} {i j : Fin n} (hk : k < n) :
      RestrictedPath k i ⟨k, hk⟩ →
      RestrictedPath k ⟨k, hk⟩ j →
      RestrictedPath (k + 1) i j

namespace RestrictedPath

/-- The min-plus weight of a restricted path. -/
def weight {n k : Nat} {i j : Fin n} (w : Matrix n) :
    RestrictedPath (n := n) k i j → Weight
  | edge i j => w i j
  | keep p => weight w p
  | through _ p q => weight w p + weight w q

/-- The intermediate vertices of a restricted path, excluding its endpoints. -/
def intermediates {n k : Nat} {i j : Fin n} :
    RestrictedPath (n := n) k i j → List (Fin n)
  | edge _ _ => []
  | keep p => intermediates p
  | @through _ k _ _ hk p q => intermediates p ++ [⟨k, hk⟩] ++ intermediates q

/--
All intermediate vertices of a `k`-restricted path lie among the first `k`
vertices.
-/
theorem mem_intermediates_lt {n k : Nat} {i j v : Fin n}
    {p : RestrictedPath (n := n) k i j} (hv : v ∈ intermediates p) :
    v.val < k := by
  induction p with
  | edge =>
...
/--
`d` is the shortest weight among the restricted paths from `i` to `j`.

The first conjunct says the bound is attained by an actual restricted path; the
second says that no restricted path has smaller weight.
-/
def IsShortestPathWeight {n k : Nat} (w : Matrix n) (i j : Fin n) (d : Weight) :
    Prop :=
  (∃ p : RestrictedPath (n := n) k i j, p.weight w = d) ∧
    ∀ p : RestrictedPath (n := n) k i j, d ≤ p.weight w
...
theorem phase_le_weight {n k : Nat} (w : Matrix n) {i j : Fin n}
    (p : RestrictedPath (n := n) k i j) :
    phase w k i j ≤ p.weight w := by
  induction p with
  | edge =>
...
/-- Each Floyd-Warshall entry is the weight of some restricted path. -/
theorem exists_weight_eq_phase {n : Nat} (w : Matrix n) (k : Nat) :
    ∀ i j : Fin n, ∃ p : RestrictedPath (n := n) k i j, p.weight w = phase w k i j := by
  induction k with
  | zero =>
      intro i j
      exact ⟨RestrictedPath.edge i j, by simp [phase, RestrictedPath.weight]⟩
  | succ k ih =>
      intro i j
      by_cases hk : k < n
      · let pivot : Fin n := ⟨k, hk⟩
        obtain ⟨pKeep, hpKeep⟩ := ih i j
        obtain ⟨pLeft, hpLeft⟩ := ih i pivot
        obtain ⟨pRight, hpRight⟩ := ih pivot j
...
theorem phase_isShortestPathWeight {n : Nat} (w : Matrix n) (k : Nat) (i j : Fin n) :
  ...
  ⟨exists_weight_eq_phase w k i j, fun p ↦ phase_le_weight w p⟩
...
theorem phase_lt_top_iff_exists_weight_lt_top {n k : Nat} (w : Matrix n) (i j : Fin n) :
    phase w k i j < ⊤ ↔
      ∃ p : RestrictedPath (n := n) k i j, p.weight w < ⊤ := by
  constructor
  · intro h
    obtain ⟨p, hp⟩ := exists_weight_eq_phase w k i j
...
theorem allPairsShortestPaths_lt_top_iff_exists_weight_lt_top {n : Nat}
    ...
    allPairsShortestPaths w i j < ⊤ ↔
      ∃ p : RestrictedPath (n := n) n i j, p.weight w < ⊤ :=
